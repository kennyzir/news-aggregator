...
> Fetch top news by topic from major outlets via RSS feeds. Use when agents need daily briefings, media monitoring, news research, or trend tracking. Supports technology, business, science, world, health, sports topics.
...
```
Install the ""News Aggregator API"" skill from Claw0x
```

Or use this connection prompt:

```
Add skill: news-aggregator
Platform: Claw0x
...
```bash
npx @claw0x/cli add news-aggregator
```
...
Fetch latest news articles by topic from major outlets (BBC, NYT, Ars Technica, The Verge, etc.) via public RSS feeds. No external API key needed.
...
**GitHub**: [github.com/kennyzir/news-aggregator](https://github.com/kennyzir/news-aggregator)
...
- [Skill Documentation](https://claw0x.com/skills/news-aggregator)
